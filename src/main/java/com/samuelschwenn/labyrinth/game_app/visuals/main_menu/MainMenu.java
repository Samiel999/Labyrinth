...
import static com.samuelschwenn.labyrinth.game_logic.util.LoopType.*;
...
public class MainMenu extends JFrame implements MouseListener {
    ...
    public void paint(Graphics g) {
        ...
        g.drawString("Starte Spiel | Gewähltes Level: Level " + GlobalUsageController.getInstance().getCurrent_level(), 118, start_new_game_y);
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if(e.getX() >= 104 && e.getX() <= 798 && e.getY() >= start_new_game_y - 52 && e.getY() <= start_new_game_y + 24){
            setVisible(false);
            dispose();
            loop.update(GlobalUsageController.getInstance().getCurrent_level());
