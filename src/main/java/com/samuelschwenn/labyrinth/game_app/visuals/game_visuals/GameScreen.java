...
import com.samuelschwenn.labyrinth.game_logic.draw_and_tickables.drawables.objects.buildings.wall.DefaultWall;
import com.samuelschwenn.labyrinth.game_logic.draw_and_tickables.drawables.objects.monster.Monster;
import com.samuelschwenn.labyrinth.game_logic.level.*;
...
import static com.samuelschwenn.labyrinth.game_logic.draw_and_tickables.drawables.objects.buildings.Building.getStaticImageOf;
...
public class GameScreen extends JPanel {
                            ...
                            if (e.getY() <= titleBarSizePixels) {
                                if (!pressed[1]) {
                                    if (e.getX() >= 300 && e.getX() <= 470) {
                                        ...
                                        LabyrinthApplication.source = true;
    ...
    public void paintComponent(Graphics g) {
        ...
        try {
            DesignLoader.getInstance().loadDesign();
    ...
    private void paintingTitleBarAndEdges(Graphics g) {
        ...
        paintTextIntoTitleBar(0, (usageController.getMoney() / 100 >= 1 ? 128 : 112), 27, "Geld: " + usageController.getMoney(), "Arial", 20, g);
    ...
    private void tick(double timeDelta) {
        ...
        for (Tickable tickable : usageController.getTickables()) {
    ...
    private void drawDrawables(Graphics g) {
        List<Drawable> drawables = usageController.getDrawables().reversed();
    ...
    private void build(Class<? extends GameObject> toBuild, CoordsInt position, double price) {
        if (price <= usageController.getMoney()) {
            if (LogicRepresentation.getInstance().getBuilding(position) == null) {
                ...
                usageController.setMoney(usageController.getMoney() - price);
                for (Monster monster : LogicRepresentation.getInstance().getMonsterList()) {
